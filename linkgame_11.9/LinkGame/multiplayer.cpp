...
//延时函数
void Multiplayer::sleep(int msec)
{
...
// 对spfa加以调整，以适应遮挡的情况
void Multiplayer::spfa(int x, int y)
{
    //计算前初始化
    for (int i = 0; i < MAXSIZE_X + 2; ++i)
        for (int j = 0; j < MAXSIZE_Y + 2; ++j)
...
bool Multiplayer::exchange(int l, int i, int j, queue<int> &q)
{
    bool flag = 1;
    int cycle = 0;
    int m = i-1, n = j;
...
// 根据dis数组，反向查找路径
vector<Nnode> Multiplayer::route()
{
    vector<Nnode> q;
...
// dfs算法搜索当前节点
bool Multiplayer::dfsRoute(vector<Nnode> &q)
{
    Nnode n = q.back();
    int col = n.x;
    int row = n.y;
...
Multiplayer::Multiplayer(QWidget *parent, QTcpSocket *tcpclient, QTcpSocket *tcphost, int roomstatus) :
    QWidget(parent),
    ...
    TcpHost(tcphost)
{
    ...
    connect(timer, &QTimer::timeout, this, &Multiplayer::timeDecrease);
    connect(this, &Multiplayer::vanish, this, &Multiplayer::timeIncrease);
    ...
    //连接传输信号至自定义读取函数
    MultiRoom multiroom;
    connect(TcpClient, SIGNAL(readyRead()), this, SLOT(HostReadData()));
    connect(TcpHost, SIGNAL(readyRead()), this, SLOT(ClientReadData()));
}
...
void Multiplayer::paintEvent(QPaintEvent *event)
{
...
void Multiplayer::mouseReleaseEvent(QMouseEvent *event)
{
...
void Multiplayer::createBlock()
{
    ...
    for(int l=0;l<layer;l++)
        for(int i=1;i<=MAXSIZE_Y;i++){
            for(int j=1;j<=MAXSIZE_X;j++){
                totalblock+=block[l][j][i];
            }
        }
    int* picAll=new int[totalblock];
    int cnt=0;
    ui->progressBarProgess->setMaximum(totalblock);
    ui->progressBarProgess_Opp->setMaximum(totalblock);

    for(int i=0;2*i<totalblock;i+=1){
        picAll[2*i]=i%16;
        picAll[2*i+1]=i%16;
    }

    srand((unsigned)time(NULL));
    random_shuffle(picAll,picAll+totalblock);
...
void Multiplayer::timeDecrease()
{
    ...
    //主机端
    if(roomStatus == 1){
        TcpClient->write("D ");  //传输格式为“D 剩余时间”
        char timelength[2];
        _itoa(timeLength, timelength, 10);
        TcpClient->write(timelength);
    }
    //客户端
    else if(roomStatus == 2){
        TcpHost->write("D ");
        char timelength[2];
        _itoa(timeLength, timelength, 10);
        TcpHost->write(timelength);
    }
    if(timeLength==0 || Efinishedblock >= totalblock)
    {
        gameStatus=6;//游戏失败
//        if(roomStatus == 1){
//            TcpClient->write("You win");
//        }
//        else if(roomStatus == 2){
//            TcpHost->write("You win");
//        }
        timer->stop();
        update();
    }
...
void Multiplayer::timeIncrease(){
    ...
    //主机端
    if(roomStatus == 1){
        TcpClient->write("I "); //传输格式为“I 剩余时间 已完成块数”
        char timelength[2];
        _itoa(timeLength, timelength, 10);
        TcpClient->write(timelength);
        TcpClient->write(" ");
        char finishedBlock[3];
        _itoa(finishedblock, finishedBlock, 10);
        TcpClient->write(finishedBlock);
    }
    else if(roomStatus == 2){ //客户端
        TcpHost->write("I ");
        char timelength[2];
        _itoa(timeLength, timelength, 10);
        TcpHost->write(timelength);
        ...
        char finishedBlock[3];
        _itoa(finishedblock, finishedBlock, 10);
        TcpHost->write(finishedBlock);
    }
    ui->progressBarProgess->setValue(finishedblock);
    ui->progressBarTimer->setValue(timeLength);
}


//void Multiplayer::on_pushButtonPause_clicked()
//{
//    if(gameStatus==4)
//    {
//        gameStatus=2;
...
void Multiplayer::on_pushButtonBackToMenu_clicked()
{
    // 传输失败标志
    if(roomStatus == 1){
        TcpClient->write("You win");
...
void Multiplayer::on_pushButtonCheat_clicked()
{
    vector<int> v=hint();
    colLast=v[0];
    rowLast=v[1];
...
// 客户端读取数据
void Multiplayer::ClientReadData()
{
    QString buffer = TcpHost->readAll();
    QString ID = buffer.split(" ")[0];  //空格号为分割标志，对字符串进行分割
    if(ID == "D"){ //时间减少
        QString time = buffer.split(" ")[1];  //剩余时间
        EtimeLength = time.toInt();
        ui->progressBarTimer_Opp->setValue(EtimeLength); //更新敌方时间条
    }
    else if(ID == "I"){ //时间增加
        QString time = buffer.split(" ")[1]; //剩余时间
        EtimeLength = time.toInt();
        ui->progressBarTimer_Opp->setValue(EtimeLength);
        QString Efb = buffer.split(" ")[2];  //完成方块数
        Efinishedblock = Efb.toInt();
        ui->progressBarProgess_Opp->setValue(Efinishedblock);   //更新敌方进度条
    }
}

// 主机函数读取数据
void Multiplayer::HostReadData()
{
    QString buffer = TcpClient->readAll();
    QString ID = buffer.split(" ")[0];
    if(ID == "D"){
        QString time = buffer.split(" ")[1];
        EtimeLength = time.toInt();
        ui->progressBarTimer_Opp->setValue(EtimeLength);
    }
    else if(ID == "I"){
        ...
        EtimeLength = time.toInt();
        ui->progressBarTimer_Opp->setValue(EtimeLength);
        QString Efb = buffer.split(" ")[2];
        Efinishedblock = Efb.toInt();
        ui->progressBarProgess_Opp->setValue(Efinishedblock);
    }
}
